...
export function Sidebar() {
  ...
  return (
    <div className="fixed inset-y-0 left-0 w-64 bg-white/30 dark:bg-zinc-900/30 backdrop-blur-md border-r border-gray-200/50 dark:border-zinc-700/50">
      <div className="flex flex-col h-full">
            ...
            {navigationItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{item.name}</span>
                  {isActive && (
                    <div className="absolute right-0 w-1 h-6 bg-gradient-to-b from-pink-500 to-violet-500 rounded-l-full shadow-lg" />
                  )}
                </Link>
              )
            })}
        ...
        {/* Footer with compact profile section */}
        <div className="p-4 border-t border-gray-200/50 dark:border-zinc-700/50 flex items-center justify-between">
          ...
          <button
            onClick={() => signOut()}
            className="flex items-center space-x-2 text-gray-600 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 transition-colors"
          >
